package com.sulake.room.renderer.cache
{
   ...
   public class BitmapDataCache
   {
      ...
      public function compress() : void
      {
         ...
         {
            this.var_411.sortOn("useCount",0 | 0);
            _loc2_ = this.var_411.length - 1;
            while(_loc2_ >= 0)
            {
               _loc1_ = this.var_411[_loc2_] as BitmapDataCacheItem;
            ...
            this.var_411.splice(_loc2_,this.var_411.length - _loc2_);
            if(this.memUsage > this.memLimit)
            {
               this.var_411.sortOn("memUsage",Array.NUMERIC);
               while(this.memUsage > this.memLimit)
               {
                  _loc1_ = this.var_411.pop() as BitmapDataCacheItem;
                  if(_loc1_ == null)
                  {
                     break;
                  }
      ...
      public function addBitmapData(param1:String, param2:ExtendedBitmapData) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            if(_loc4_ != null)
            {
               this.var_759 -= _loc4_.width * _loc4_.height * 4;
            }
         ...
         else
         {
            ...
            this.var_411.push(_loc3_);
         }
         this.var_759 += param2.width * param2.height * 4;
      }
   }
}
