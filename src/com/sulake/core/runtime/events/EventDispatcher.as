package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_558:uint = 0;
      
      private static const const_1236:uint = 1;
      
      private static const const_878:uint = 2;
       
      
      protected var _disposed:Boolean = false;
      
      private var var_216:flash.events.EventDispatcher;
      
      private var var_238:Dictionary;
      
      private var _result:uint;
      
      private var var_593:Error;
      
      public function EventDispatcher(param1:IEventDispatcher = null)
      {
         this.var_238 = new Dictionary();
         super();
         this.var_216 = new flash.events.EventDispatcher(!!param1 ? param1 : this);
      }
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         if(!_loc6_)
         {
            ...
            this.var_216.addEventListener(param1,this.eventProcessor);
         }
         else
         {
            for each(_loc8_ in _loc6_)
            {
               if(_loc8_.callback == param2 && _loc8_.var_1881 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            ...
            if(_loc4_)
            {
               ...
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1881 == param3)
                  {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            this._result = const_558;
            this.var_216.dispatchEvent(param1);
            if(this._result == const_878)
            {
               if(this.var_593 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_593.message,this.var_593.errorID,this.var_593);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_593);
               }
            }
            return this._result == const_558;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1236) : uint(const_558);
      }
