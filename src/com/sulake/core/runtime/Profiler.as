package com.sulake.core.runtime
{
   ...
   class Profiler extends Component implements IProfiler
   {
      ...
      private var var_268:Dictionary;
      ...
      {
         this.var_268 = new Dictionary(true);
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            for(_loc1_ in this.var_268)
            {
               IDisposable(this.var_268[_loc1_]).dispose();
               delete this.var_268[_loc1_];
            }
            super.dispose();
         }
      }
      ...
      {
         this.var_2660 = getTimer() - this.var_2089;
      ...
      public function getProfilerAgentsInArray() : Array
      {
         ...
         for(_loc3_ in this.var_268)
         {
            _loc2_ = _loc3_ as IUpdateReceiver;
            if(IDisposable(_loc3_).disposed)
            {
               IDisposable(this.var_268[_loc3_]).dispose();
               delete this.var_268[_loc3_];
            }
            else
            {
               _loc1_.push(this.var_268[_loc3_]);
            }
         }
         return _loc1_;
      }
      ...
      {
         var _loc2_:ProfilerAgent = this.var_268[param1];
