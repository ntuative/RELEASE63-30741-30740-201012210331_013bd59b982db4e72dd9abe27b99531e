package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1400:int = 4;
      ...
      {
         return this.var_386.length + this.var_255.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_255.length < const_1400 && this.var_386.length > 0)
            {
               _loc1_ = this.var_386.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_386.indexOf(loader);
            if(index > -1)
            {
               this.var_386.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_386)
            {
               if(param2)
               {
