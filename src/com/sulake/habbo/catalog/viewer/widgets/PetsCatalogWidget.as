package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_917 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_344 = [];
         this.var_618 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_917,this);
               if(_loc9_)
               {
                  this.var_618 = _loc9_.sellableBreeds;
                  this.var_344 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_618.length)
         {
            _loc10_ = this.var_618[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_917,_loc10_.id),this.getRaceLocalizationKey(this.var_917,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_917,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_912,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_344)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_501 = param1.index;
         if(this.var_501 < 0 || this.var_501 > this.var_344.length)
         {
            this.var_501 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_501 >= 0 && this.var_501 < this.var_344.length)
         {
            _loc7_ = this.var_344[this.var_501] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_618[this.var_1832] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_917,_loc3_.id,_loc2_,AvatarScaleType.const_46,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_277,AvatarAction.const_565);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_501 >= this.var_344.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_344[this.var_501] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_618[this.var_1832] as IBreed;
