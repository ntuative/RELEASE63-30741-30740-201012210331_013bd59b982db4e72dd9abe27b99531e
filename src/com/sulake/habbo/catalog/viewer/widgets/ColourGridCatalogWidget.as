package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class ColourGridCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      override public function init() : void
      {
         ...
         this.var_1844 = _loc1_.content as XML;
      ...
      private function populateColourGrid() : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = 0;
         Logger.log("[ColourGridCatalogWidget] Display colors: " + [this.var_1405.length,this._colourGrid.numGridItems]);
         this._colourGrid.destroyGridItems();
         this.var_340 = null;
         for each(_loc1_ in this.var_1405)
         {
            _loc2_ = page.viewer.catalog.windowManager.buildFromXML(this.var_1844) as IWindowContainer;
            _loc2_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onClick);
            _loc2_.background = true;
            _loc2_.color = 4294967295;
            _loc2_.width = this.var_752.width;
            _loc2_.height = this.var_752.height;
            this._colourGrid.addGridItem(_loc2_);
            _loc3_ = _loc2_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc3_ != null)
            {
               _loc3_.bitmap = new BitmapData(this.var_752.width,this.var_752.height,true,0);
               _loc3_.bitmap.copyPixels(this.var_752,this.var_752.rect,new Point(0,0));
            }
            _loc4_ = _loc2_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_973.width,this.var_973.height,true,0);
               _loc6_ = 255;
               _loc7_ = 255;
               _loc8_ = 255;
               if(_loc1_ > 0)
            ...
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_800.width,this.var_800.height,true,16777215);
               _loc5_.bitmap.copyPixels(this.var_800,this.var_800.rect,new Point(0,0),null,null,true);
