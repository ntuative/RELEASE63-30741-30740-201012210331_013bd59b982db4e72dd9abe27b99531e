package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      private var var_148:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_127:SSOTicket;
      ...
      {
         this.var_148.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_148 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_148.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_116,HabboWindowStyle.const_37,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_148.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1542,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_148.habboConfiguration)
            {
               if(this.var_148.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_148.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_148.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1542,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_148.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_127.removeEventListener(SSOTicket.const_315,this.onParsedTicketSuccess);
            this.var_127.removeEventListener(SSOTicket.const_304,this.onParsedTicketFailure);
            this.var_127 = null;
         }
         var _loc2_:String = "";
         if(this.var_148.habboConfiguration != null)
         {
            _loc2_ = this.var_148.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_148.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_127 = new SSOTicket(this.var_148.assets,_loc3_,_loc2_,param1);
         this.var_127.addEventListener(SSOTicket.const_315,this.onParsedTicketSuccess);
         this.var_127.addEventListener(SSOTicket.const_304,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_127.ticket);
         Logger.log("Has Facebook: " + this.var_127.isFacebookUser);
         if(this.var_127.isFacebookUser)
         {
            this.var_148.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_148.shockwaveClientUrl = this.var_127.shockwaveClientUrl;
         this.var_148.ssoTicket = this.var_127.ticket;
         this.var_127.removeEventListener(SSOTicket.const_315,this.onParsedTicketSuccess);
         this.var_127.removeEventListener(SSOTicket.const_304,this.onParsedTicketFailure);
         this.var_127 = null;
         dispatchEvent(new Event(const_503));
