package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      private var var_1230:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String, param4:Boolean = true)
      {
         ...
         this.var_1229 = "http://" + param3 + "/client";
         if(!param4)
         {
            this.var_1230.init();
            this.var_1230.addEventListener(HabboWeb.const_242,this.requestClientURL);
         }
         else
         {
      ...
      {
         var _loc2_:String = this.var_1229;
         if(this._assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + this.var_1229);
         }
         var _loc3_:URLRequest = new URLRequest(this.var_1229);
         if(this._assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            this.var_1230.requestReAuthenticate();
         }
         else
         {
            try
            {
               this.var_1513 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               this.var_2011 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               facebookData = /\"facebook.user\" : \"(.*?)\"/.exec(data);
               if(facebookData)
               {
                  this.var_2010 = Boolean(facebookData[1]);
               }
               if(this.var_1513.length > 0)
               {
