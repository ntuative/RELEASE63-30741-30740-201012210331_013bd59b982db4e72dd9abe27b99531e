package com.sulake.habbo.communication.messages.incoming.navigator
{
   import com.sulake.core.communication.messages.IMessageDataWrapper;
   import com.sulake.core.runtime.IDisposable;
   
   public class OfficialRoomEntryData implements IDisposable
   {
      
      public static const const_941:int = 1;
      
      public static const const_764:int = 2;
      
      public static const const_673:int = 3;
      
      public static const const_1293:int = 4;
       
      
      private var _index:int;
      
      private var var_2008:String;
      
      private var var_2004:String;
      
      private var var_2009:Boolean;
      
      private var var_2007:String;
      
      private var var_866:String;
      
      private var var_2005:int;
      
      private var var_1983:int;
      
      private var _type:int;
      
      private var var_2006:String;
      
      private var var_803:GuestRoomData;
      
      private var var_802:PublicRoomData;
      
      private var _open:Boolean;
      
      private var _disposed:Boolean;
      
      public function OfficialRoomEntryData(param1:IMessageDataWrapper)
      {
         super();
         this._index = param1.readInteger();
         this.var_2008 = param1.readString();
         this.var_2004 = param1.readString();
         this.var_2009 = param1.readInteger() == 1;
         this.var_2007 = param1.readString();
         this.var_866 = param1.readString();
         this.var_2005 = param1.readInteger();
         this.var_1983 = param1.readInteger();
         this._type = param1.readInteger();
         if(this._type == const_941)
         {
            this.var_2006 = param1.readString();
         }
         else if(this._type == const_673)
         {
            this.var_802 = new PublicRoomData(param1);
         }
         else if(this._type == const_764)
         {
            this.var_803 = new GuestRoomData(param1);
         }
         else
         {
            this._open = param1.readBoolean();
         }
      }
      
      public function dispose() : void
      {
         if(this._disposed)
         {
            return;
         }
         this._disposed = true;
         if(this.var_803 != null)
         {
            this.var_803.dispose();
            this.var_803 = null;
         }
         if(this.var_802 != null)
         {
            this.var_802.dispose();
            this.var_802 = null;
         }
      }
      
      public function get disposed() : Boolean
      {
         return this._disposed;
      }
      
      public function get type() : int
      {
         return this._type;
      }
      
      public function get index() : int
      {
         return this._index;
      }
      
      public function get popupCaption() : String
      {
         return this.var_2008;
      }
      
      public function get popupDesc() : String
      {
         return this.var_2004;
      }
      
      public function get showDetails() : Boolean
      {
         return this.var_2009;
      }
      
      public function get picText() : String
      {
         return this.var_2007;
      }
      
      public function get picRef() : String
      {
         return this.var_866;
      }
      
      public function get folderId() : int
      {
         return this.var_2005;
      }
      
      public function get tag() : String
      {
         return this.var_2006;
      }
      
      public function get userCount() : int
      {
         return this.var_1983;
      }
      
      public function get guestRoomData() : GuestRoomData
      {
         return this.var_803;
      }
      
      public function get publicRoomData() : PublicRoomData
      {
         return this.var_802;
      }
      
      public function get open() : Boolean
      {
         return this._open;
      }
      
      public function toggleOpen() : void
      {
         this._open = !this._open;
      }
      
      public function get maxUsers() : int
      {
         if(this.type == const_941)
         {
            return 0;
         }
         if(this.type == const_764)
         {
            return this.var_803.maxUserCount;
         }
         if(this.type == const_673)
         {
            return this.var_802.maxUsers;
         }
         return 0;
      }
   }
}
