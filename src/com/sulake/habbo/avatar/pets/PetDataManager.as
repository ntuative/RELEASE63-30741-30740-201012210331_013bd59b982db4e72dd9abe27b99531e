package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_400.getValue(_loc4_) == null)
               {
                  this.var_400.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         if(param1 == null || this.var_500 == null)
         {
            return;
         }
         var _loc2_:int = this.var_500.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_500.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_500.length > 0)
         {
            _loc1_ = this.var_500.pop() as IPetDataListener;
      ...
      {
         var _loc3_:IPetData = this.var_400.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      {
         var _loc4_:PetData = this.var_400.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_400.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_400.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.const_1812.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1394.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_617.getValue(_loc6_) == null)
               {
                  ...
                  this.var_617.add(_loc6_,_loc8_);
                  if(param3 != null && this.var_500.indexOf(param3) == -1)
                  {
                     this.var_500.push(param3);
                  }
               }
            }
         }
      ...
      private function unregisterLoadingAssets(param1:LibraryLoader) : void
      {
         ...
         for each(_loc3_ in this.var_617.getKeys())
         {
            _loc5_ = this.var_617.getValue(_loc3_);
            if(param1 == _loc5_)
            {
               _loc2_.push(_loc3_);
            }
