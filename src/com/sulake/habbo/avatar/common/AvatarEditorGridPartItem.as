package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_632:IFigurePartSet;
      ...
      private var var_523:Rectangle;
      
      private var var_1453:int = 0;
      
      private var var_356:IAvatarRenderManager;
      
      private var var_679:Boolean;
      
      private var var_1452:BitmapData;
      
      private const const_1815:int = 13421772;
      
      private const const_1814:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_625 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1453 = Math.max(this.var_1453,_loc6_.colorLayerIndex);
            }
         }
         this.var_356 = this.var_62.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_625 != null && !this.var_1451)
            {
               _loc3_ = this.var_625;
            }
            else
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:AvatarFigureContainer = this.var_356.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_356.isFigureReady(_loc1_))
         {
            this.var_356.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_309[_loc6_] + "_" + FigureData.const_116;
               _loc4_ = this.var_356.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_523 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1452)
               {
                  ...
                  this.var_1452 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_523.width,this.var_523.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_523.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_523.y;
               if(this.var_1451 && _loc2_.colorLayerIndex > 0)
               {
