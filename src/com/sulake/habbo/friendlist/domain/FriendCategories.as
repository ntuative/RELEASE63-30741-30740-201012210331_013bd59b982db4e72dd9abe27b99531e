package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_360:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_358);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_360.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_360.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_360.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_360.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_360.view.setNewMessageArrived();
                  this.var_360.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_358).received = true;
         this.findCategory(FriendCategory.const_1023).received = true;
