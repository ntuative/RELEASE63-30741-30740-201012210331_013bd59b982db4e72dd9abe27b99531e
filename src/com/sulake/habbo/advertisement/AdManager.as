package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_1042:BitmapData = null;
      
      private var var_1041:BitmapData = null;
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2188 = this._config.getKey("interstitial.enabled","false") == "true";
         this.var_2186 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2189 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1629 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_201 != null && this.var_201.running)
         {
            this.var_201.reset();
            return false;
         }
         if(this.var_1040 == 0)
         {
            this.var_1040 = getTimer();
            return false;
         }
         if(this.var_1629 > 0 && this.var_1630 >= this.var_1629)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1040;
         if(_loc1_ > this.var_2186)
         {
            ++this.var_1630;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1298 = _loc2_.imageUrl;
         this.var_2187 = _loc2_.clickUrl;
         this.var_1040 = getTimer();
         if(this.var_1298 != "")
         {
            _loc3_ = this.var_1298;
            ...
            {
               this.var_201 = new Timer(this.var_2189,1);
               this.var_201.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_201.running)
            {
               this.var_201.reset();
            }
            this.var_201.start();
            _loc4_ = new URLRequest(this.var_1298);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_474,0,0,param1.bitmapData,this.var_2187));
            this.var_1040 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_491));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1297 = _loc2_.imageUrl;
         this.var_1628 = _loc2_.clickUrl;
         if(this.var_1297 != "" && this.var_1628 != "")
         {
            _loc3_ = this.var_1297;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1297);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_513,0,0,_loc4_,this.var_1628,this.var_1042,this.var_1041));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1042 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
      ...
      private function adWarningRightReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1041 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
